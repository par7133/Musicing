...
// PRIVATE_VIEW="3" 
 ...
 $reqView = substr(strip_tags(filter_input(INPUT_POST, "_reqView")??""), 0, 1);
 //echo("reqView=*".$reqView."*<br>");
 if (($reqView !== "") && (($reqView !== "1") && ($reqView !== "3"))) {
   //echo("pippo<br>");
   $password = PHP_STR;
 }
  
 if ($password !== PHP_STR) {
   if ($reqView === ADMIN_VIEW) {
     $CURRENT_VIEW = ADMIN_VIEW;
   } else {
     $CURRENT_VIEW = PRIVATE_VIEW;
   }  
 } else {
   $CURRENT_VIEW = PUBLIC_VIEW;
 } 

 //echo("CURRENT_VIEW=".$CURRENT_VIEW."<br>");
 ...
 function uploadNewRes() {
   ...
   //error_log("pippo=1");
   //echo(is_uploaded_file($_FILES['filesdd']['tmp_name']));
   //exit(1);
   //error_log("tmp_name0 = ");
   ...
   if (!empty($_FILES['files']['tmp_name'][0]) ||  !empty($_FILES['filesdd']['tmp_name'][0])) {
     ...
     foreach($uploads as &$upload) {
       ...
       //echo_ifdebug(true, "EXT#1=");
       //echo_ifdebug(true, $fileExt);
              
       $date = date("Ymd-His");
       $rnd = mt_rand(1000000000, 9999999999);    
   
       if ($fileExt === "mp3") { 
         $ipos = stripos($originalFilename, "~");
         
         $originalPlaylistName = trim(left($originalFilename, $ipos));
         // case standardization
         $originalPlaylistName = ucfirst(strtolower($originalPlaylistName));
         
         $pattern = $BLOG_PATH . DIRECTORY_SEPARATOR . "*" . "|" . str_replace(" ", "_", $originalPlaylistName);
         $aExistingPlaylists = glob($pattern, GLOB_ONLYDIR);
         if (!empty($aExistingPlaylists)) {
           $playlistName = basename($aExistingPlaylists[0]);  
         } else {
           $playlistName = $date . "-" . $rnd . substr($google, $i-1, 1) . "|" . str_replace(" ", "_", $originalPlaylistName);
         }
                  
         $originalSongTitle = trim(substr($originalFilename, $ipos+1));
              
         $destFileName = str_replace(" ", "_", $originalSongTitle) . ".$fileExt";
         
       } else if ($originalFileExt!==PHP_STR) {
       ...
       if (empty($destPaths)) {
          ...
          switch ($fileExt) {
            ...
            case "pdf":
              ...
              break;
            case "mp3":
              $destPaths[] = $BLOG_PATH . DIRECTORY_SEPARATOR . $playlistName;
              break;
            case "png":
            case "jpg":
            case "jpeg":
       ...
       foreach($destFullPaths as $destFullPath) {
          ...
          if (!is_readable($destPaths[$iPath])) {
            mkdir($destPaths[$iPath], 0777, true); 
          }

          if ($fileExt === "mp3") {
            $pattern = $BLOG_PATH . DIRECTORY_SEPARATOR . "*" . "|" . str_replace(" ", "_", $originalPlaylistName) . DIRECTORY_SEPARATOR . $originalSongTitle  . ".mp3";
          } else { 
 ...
 function displaySongTitle($playlist, $song, $oriplaylistname, $orisongfilename) {
   
   global $CURRENT_VIEW;
   
   $run1 = "playSong(\"$oriplaylistname\", \"$orisongfilename\");event.stopPropagation();";
   $run2 = "stopSong();event.stopPropagation();";
   ...
   if ($CURRENT_VIEW === PRIVATE_VIEW) {
     echo("<div style='height:24px;clear:both;border-bottom:1px dashed gray;'>" . "<div style='float:left'>".$song."</div>" . "<div style='float:right'><a class='playf".$playlist."' href='#' onclick='$run1'>"."Play"."</a>&nbsp;<a href='#' onclick='$run2'>"."Stop"."</a></div>" . "</div>");         
   } else {
     echo("<div style='height:24px;clear:both;border-bottom:1px dashed gray;'>" . "<div style='float:left'>".$song."</div>" . "<div style='float:right'>"."Play"."</div>" . "</div>");         
   }  
 }
...
<?PHP if ($CURRENT_VIEW == ADMIN_VIEW): ?>    
  ...
  <body style="background:url('/res/bg1.jpg') no-repeat; background-size: cover; background-attachment: fixed; background-position: center;">
   <div id="AFHint" style="z-index:0;margin-top:58px;">
...
<?php endif; ?>
   ...
   <audio id="myPlayer" preload="auto">
      <source src="/media/sample.mp3" type="audio/mpeg">
       Maybe your browser doesn't support the audio..
   </audio>  
 ...
 <div id="blog">    
   ...
   $aPlaylistPaths = glob($pattern, GLOB_ONLYDIR);   
   if (empty($aPlaylistPaths)): ?>
             <div class="blog-content"> 
              <div class="blog-entry" style="background-color:#FFFFFF;">    
                <br><br>
                <?PHP echo(date("Ymd") . " " . date("Hi"));?><br><br><br>
                <?PHP echo(getResource0("Demo playlist", $lang));?><br><br>
                <?PHP displaySongTitle(0,null,null,null);?>
              </div> 
             </div>  
          <?PHP $totLinks = 1; ?>
        <?PHP else: ?>
      ...
      $iEntry = 1;          
      $iCurEntry = 1; 
      arsort($aPlaylistPaths, SORT_STRING);
      $totPlaylist = count($aPlaylistPaths);
      ...
      //echo("blogSP=".$blogSP);
      foreach ($aPlaylistPaths as $playlistPath) {
        ...
        $playlistName = str_replace("_", " ", explode("|",$oriplaylistname)[1]);
        ...
        ?>
             ...
             if (empty($aSongPaths)) {
               ...
               foreach ($aSongPaths as $songPath) {

                 $orisongfilename = basename($songPath);
                 $orifileExt = strtolower(pathinfo($orisongfilename, PATHINFO_EXTENSION)); 

                 $songName = pathinfo($orisongfilename, PATHINFO_FILENAME);

                 ?>
                                   
                                              <?PHP if ($orifileExt === "mp3"):?>      
                                                      <?PHP displaySongTitle($iEntry,$songName,$oriplaylistname,$orisongfilename); ?>
                                             <?PHP else: ?>
                                                      &nbsp;
                                            <?PHP endif; ?>      
                                                      
                                            <?PHP $aLinks[] = "/img?av=" . AVATAR_NAME . "&pic=" . $songName; ?>
  ...
  function selectVideo(i) {
    ...
    $(".modal-content").each(function(){
      if (y==i) {
        ...
        s=s.replaceAll("playList("+(y+1)+",false, true);","playList("+(y+1)+",<?PHP echo(($CURRENT_VIEW == PUBLIC_VIEW)?"false":"true");?>, true);");
        //s=s.replaceAll("class='play-list-click2'","class='play-list-click' onclick='playList();'");
        s=s.replaceAll("play-list-img2","play-list-img");
